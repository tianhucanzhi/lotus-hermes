...
def _set_process_title() -> None:
    ...
    try:
        ...
        from hermes_constants import CLI_COMMAND

        setproctitle.setproctitle(CLI_COMMAND)
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, CLI_COMMAND.encode(), 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            from hermes_constants import CLI_COMMAND

            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(CLI_COMMAND.encode())
        # Windows: the .exe name is already ``lotus.exe`` — nothing to do.
    except Exception:
        pass
...
def _print_fast_version_info() -> None:
    ...
    print(f"{AGENT_DISPLAY_NAME} v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print(f"    1. Start the gateway:  {CLI_COMMAND} gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print(f"  Tip: Agent responses are prefixed with '⚕ {AGENT_DISPLAY_NAME}'")
        else:
            from hermes_constants import AGENT_DISPLAY_NAME, CLI_COMMAND
        ...
        print(f"  Or install as a service: {CLI_COMMAND} gateway install")
    else:
        from hermes_constants import CLI_COMMAND

        print(f"⚠ Pairing may not have completed. Run '{CLI_COMMAND} whatsapp' to try again.")
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print(f"Install Node.js, then run:  {CLI_COMMAND} desktop")
            sys.exit(1)
    else:
        npm = None
    ...
    if source_mode:
        print(f"→ Launching {AGENT_DISPLAY_NAME} Desktop from source build...")
    ...
    print(f"→ Launching packaged {AGENT_DISPLAY_NAME} Desktop: {packaged_executable}")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print(f"⚕ Updating {AGENT_DISPLAY_NAME}...")
    print()
