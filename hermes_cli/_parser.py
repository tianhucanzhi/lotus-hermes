...
from hermes_constants import AGENT_DISPLAY_NAME, CLI_COMMAND
...
def _build_epilogue() -> str:
    ...
    return f"""
Examples:
    {c}                        Start interactive chat
    {c} chat -q "Hello"        Single query mode
    {c} --tui                  Launch the modern TUI (or set display.interface: tui)
    {c} --cli                  Force the classic REPL (overrides display.interface: tui)
    {c} -c                     Resume the most recent session
    {c} -c "my project"        Resume a session by name (latest in lineage)
    {c} --resume <session_id>  Resume a specific session by ID
    {c} setup                  Run setup wizard
    {c} logout                 Clear stored authentication
    {c} auth add <provider>    Add a pooled credential
    {c} auth list              List pooled credentials
    {c} auth remove <p> <t>    Remove pooled credential by index, id, or label
    {c} auth reset <provider>  Clear exhaustion status for a provider
    {c} model                  Select default model
    {c} fallback [list]        Show fallback provider chain
    {c} fallback add           Add a fallback provider (same picker as `{c} model`)
    {c} fallback remove        Remove a fallback provider from the chain
    {c} config                 View configuration
    {c} config edit            Edit config in $EDITOR
    {c} config set model gpt-4 Set a config value
    {c} gateway                Run messaging gateway
    {c} -s lotus-agent-dev,github-auth
    {c} -w                     Start in isolated git worktree
    {c} gateway install        Install gateway background service
    {c} sessions list          List past sessions
    {c} sessions browse        Interactive session picker
    {c} sessions rename ID T   Rename/title a session
    {c} logs                   View agent.log (last 50 lines)
    {c} logs -f                Follow agent.log in real time
    {c} logs errors            View errors.log
    {c} logs --since 1h        Lines from the last hour
    {c} debug share             Upload debug report for support
    {c} update                 Update to latest version
    {c} dashboard              Start web UI dashboard (port 9119)
    {c} dashboard --stop       Stop running dashboard processes
    {c} dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog=CLI_COMMAND,
        description=f"{AGENT_DISPLAY_NAME} - AI assistant with tool-calling capabilities",
