...
# ASCII Art - Lotus caduceus hero (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = f"⚕ {AGENT_DISPLAY_NAME.upper()} - AI Agent Framework"
        tiny_line = f"⚕ {AGENT_DISPLAY_NAME.upper()}"
    else:
        agent_name = _skin.get_branding("agent_name", AGENT_DISPLAY_NAME) if _skin else AGENT_DISPLAY_NAME
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"{AGENT_DISPLAY_NAME} v{_version} ({_release_date})"
    else:
...
class HermesCLI:
    ...
    def run(self):
        ...
        def handle_ctrl_z(event):
            ...
            agent_name = get_active_skin().get_branding("agent_name", AGENT_DISPLAY_NAME)
