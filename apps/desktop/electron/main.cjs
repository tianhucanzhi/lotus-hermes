...
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', `${CLI_COMMAND}.exe`)
    : path.join(updateRoot, 'venv', 'bin', CLI_COMMAND)
}
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      const updateRoot = resolveUpdateRoot()
      let command = `${CLI_COMMAND} update`
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `${CLI_COMMAND} update --branch ${branch}`
        }
      } catch {
    ...
    emitUpdateProgress({ stage: 'restart', message: `Handing off to the ${APP_NAME} updater…`, percent: 100 })
...
function resolveHermesCliBinary(updateRoot) {
  const venvCli = IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', `${CLI_COMMAND}.exe`)
    : path.join(updateRoot, 'venv', 'bin', CLI_COMMAND)
  if (fileExists(venvCli)) return venvCli
  return findOnPath(CLI_COMMAND) || null
}
...
async function applyUpdatesPosixInApp() {
  ...
  if (!hermes) {
    const manualUpdate = `${CLI_COMMAND} update`
    emitUpdateProgress({ stage: 'manual', message: manualUpdate, percent: null })
    return { ok: true, manual: true, command: manualUpdate, hermesRoot: updateRoot }
  }
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: `${CLI_COMMAND} update failed.`, error: updated.error || 'update-failed' })
    return { ok: false, error: `${CLI_COMMAND} update failed` }
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ${CLI_COMMAND} desktop --force-build`
  )
  return candidates[0]
}
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      hermesCommand = findOnPath(CLI_COMMAND)
    }
