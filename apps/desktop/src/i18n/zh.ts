...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 Lotus 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Lotus 桌面版…'
    },
    errors: {
      backgroundExited: 'Lotus 后台进程已退出。',
      backgroundExitedDuringStartup: 'Lotus 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Lotus 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    updateHermes: '更新 Lotus',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Lotus Desktop。',
  ...
  titlebar: {
    ...
    openRealApiTitle: 'Real-Api聚合全球 最顶级AI 模型-浏览器中打开',
    editionPersonal: '个人版',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Lotus 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Lotus 跟随系统设置。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Lotus 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Lotus 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Lotus 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Lotus 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Lotus 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Lotus 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Lotus Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Lotus 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Lotus 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Lotus 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Lotus Desktop 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Lotus ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Lotus 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
    ...
    customProviders: {
      loading: '正在加载自定义端点…',
      intro: '将 OpenAI 兼容 API 保存到 config.yaml 的 custom_providers（vLLM、Ollama、私有 API 等）。',
      addButton: '添加端点',
      addTitle: '新建自定义端点',
      editTitle: '编辑自定义端点',
      empty: '还没有自定义端点。添加后可让 Lotus 连接你自己的 API。',
      nameLabel: '显示名称',
      namePlaceholder: '我的 API',
      baseUrlLabel: 'Base URL',
      baseUrlHint: 'OpenAI 兼容根地址，通常以 /v1 结尾',
      baseUrlRequired: '请先填写 Base URL。',
      apiKeyLabel: 'API 密钥',
      apiKeyHint: '可选。编辑时留空表示保留现有密钥。',
      apiKeyPlaceholder: 'sk-…（可选）',
      apiKeyKeepPlaceholder: '留空则保留当前密钥',
      apiKeySet: '已设置 API 密钥',
      modelLabel: '模型',
      modelHint: '发给 API 的模型 id。可点「测试」从 /v1/models 自动发现。',
      modelPlaceholder: 'model-name',
      modelRequiredForMain: '设为主模型前请先填写模型名称。',
      noModelSet: '未设置默认模型',
      apiModeLabel: '协议模式',
      apiModeHint: '一般选「自动」即可，除非端点需要特定 API 格式。',
      apiModeAuto: '自动',
      ...
      probeButton: '测试',
      probeOk: '端点可达。',
      probeFound: count => `发现 ${count} 个模型。`,
      probeUnreachable: '无法连接该端点。',
      useAsMain: '设为主模型',
      saved: '自定义端点已保存。',
      saveFailed: '无法保存自定义端点。',
      saving: '保存中…',
      requiredFields: '名称和 Base URL 为必填项。',
      deleteConfirm: name => `删除自定义端点「${name}」？`,
      appliedMain: name => `主模型已切换为 ${name}。`,
      applyFailed: '无法将该端点设为主模型。'
    },
    sessions: {
      loading: '正在加载已归档会话…',
      archivedTitle: '已归档会话',
  ...
  skills: {
    ...
    hub: {
      ...
      connectedHubs: '已连接来源：',
      connecting: '正在连接技能市场…',
      detailDescription: name => `安装前预览并扫描 ${name}。`,
      details: '详情',
      dismiss: '关闭',
      emptyLanding: '在上方搜索，浏览可安装的技能。',
      featuredHint: '来自 Hermes 索引 — 搜索可发现更多',
      featuredTitle: '精选技能',
      files: '文件',
      install: '安装',
      installCompleteMessage: '可在「技能」标签中启用，新会话生效。',
      installCompleteTitle: '技能已安装',
      installFailed: '安装失败',
      installStarting: identifier => `正在安装 ${identifier}…`,
      installed: '已安装',
      loading: '正在加载技能市场…',
      loadingPreview: '正在加载 SKILL.md…',
      noFindings: '未发现安全问题。',
      noResults: '市场中未找到匹配的技能。',
      openSkill: name => `打开 ${name}`,
      policyBlock: '策略阻止安装',
      previewEmpty: '（SKILL.md 为空）',
      previewFailed: '预览失败',
      previewLoadFailed: '无法加载技能内容。',
      readSkillMd: '阅读 SKILL.md',
      rescan: '重新扫描',
      scanFailed: '安全扫描失败',
      scanPrompt: '安装前可运行安全扫描。',
      scanning: '正在获取并扫描…',
      search: '搜索',
      searchFailed: '市场搜索失败',
      searchMeta: (count, ms) => `${count} 条结果${ms != null ? ` · ${ms}ms` : ''}`,
      searchPlaceholder: '搜索技能市场…',
      searching: '正在搜索市场…',
      securityScan: '安全扫描',
      sourcesFallback: '结果与 hermes skills search 使用相同来源。',
      timedOut: sources => `${sources} 请求超时`,
      trust: {
        trusted: '可信',
        builtin: '内置',
        community: '社区',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Lotus 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Lotus ${version} · 活跃会话 ${count}`,
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Lotus 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Lotus 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Lotus 指向该 URL。',
      ...
      weixin: '登录微信公众平台，复制 AppID 和 Token，并把消息回调 URL 指向 Lotus。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Lotus 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Lotus 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Lotus 会运行它，并把结果发送到你选择的目的地。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Lotus…',
    placeholderReconnecting: '正在重新连接 Lotus…',
    ...
    attachUrlDesc: 'Lotus 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Lotus 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Lotus 已可安装。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Lotus，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Lotus 时会使用新版本。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody: 'Lotus 更新器会在自己的窗口中接管，并在完成后重新打开 Lotus。',
    applyingClose: 'Lotus 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Lotus Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Lotus CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Lotus 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Lotus 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    ...
    preparingInstall: 'Lotus 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Lotus…',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Lotus 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Lotus 会自动连接',
      loopback: '打开浏览器登录 — Lotus 会自动连接',
    ...
    authorizeThere: '请在那里授权 Lotus。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Lotus，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Lotus 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Lotus 正在重启...',
      askRestart: '让 Lotus 重启服务器',
      lookingRestart: taskId => `Lotus 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Lotus 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `Lotus 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Lotus 无法重启服务器。',
      stillWorking: 'Lotus 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'Lotus 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      moreOptions: '更多审批选项',
      allowSession: '允许本会话',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。Lotus 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Lotus 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Lotus 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Lotus 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Lotus 桌面版以保存图片',
